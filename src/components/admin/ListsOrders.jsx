...
const ListsOrders = () => {
  ...
  const setOrders = () => {
    ...
    data?.orders?.forEach((order) => {
      // Check if the order is COD and is delivered
      const paymentStatus = order?.paymentMethod === "COD" && order?.orderStatus === "Delivered" ? "Paid" : order?.paymentInfo?.status?.toUpperCase();

      orders.rows.push({
        id: order?._id,
        paymentStatus: (
          <span className={`badge ${paymentStatus === "PAID" || paymentStatus === "Paid" ? "bg-success" : "bg-warning text-dark"}`}>
            {paymentStatus}
          </span>
        ),
        orderStatus: (

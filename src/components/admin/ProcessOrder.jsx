...
const ProcessOrder = () => {
    ...
    useEffect(() => {
        ...
        if (updateError) toast.error(updateError?.data?.message);
        if (isSuccess) toast.success("Order updated successfully!");
    ...
    // Handle COD logic on the frontend for display
    const displayPaymentStatus = 
      paymentMethod === "COD" && orderStatus === "Delivered"
        ? "PAID"
        : paymentInfo?.status?.toUpperCase() || "NOT PAID";

    const paymentBadgeColor = displayPaymentStatus === "PAID" ? "bg-success" : "bg-warning text-dark";
      ...
      <AdminLayout>
        ...
        <div className="row d-flex justify-content-around">
            ...
            <table className="table table-striped table-bordered">
              <tbody>
                <tr>
                  ...
                  <td>
                    <b className={`badge ${paymentBadgeColor}`}>{displayPaymentStatus}</b>
                  </td>
                </tr>
                <tr>
              ...
              {orderItems?.map((item) => (
                <div className="row my-5" key={item._id}>
                  ...
                  <div className="col-5 col-lg-5">
                    <Link to={`/foods/${item.food}`} className="text-decoration-none">
                      {item.name}
                    </Link>
                  </div>
          ...
          <div className="col-12 col-lg-3 mt-5">
            ...
            <button onClick={() => updateOrderHandler(order?._id)} className="btn btn-danger w-100">
              Update Status
            </button>
